...
function device_icon(nodeId, withText) {
	...
	ico.find('.device_icon_img').bind('error', function() {
	...
	}).bindPath('devices[' + nodeId + '].instances[0].commandClasses[' + 0x25 + '].data.level,devices[' + nodeId + '].instances[0].commandClasses[' + 0x26 + '].data.level,devices[' + nodeId + '].instances[0].commandClasses[' + 0x30 + '].data[1],deviceIconsUpdateManually', function(obj, path, icon_nodeId) {
		...
		if (icon_nodeId == 255)
			...
			switch (genericType) {
				...
				case 0x20:
					if (0x30 in ZWaveAPIData.devices[icon_nodeId].instances[0].commandClasses && ZWaveAPIData.devices[icon_nodeId].instances[0].commandClasses[0x30].data[1] && ZWaveAPIData.devices[icon_nodeId].instances[0].commandClasses[0x30].data[1].level.value)
						icon_name_suffix += '_255';
					else
						icon_name_suffix += '_0';
					break;
...
function showConfigUISimpleDevice(nodeId) {
	...
	if (0x70 in node.instances[0].commandClasses) {
		...
		ZDDX(nodeId).find('configparams configparam').each(function (i, conf_html) {
			...
			var param_cont = $('<div class="configuration_parameter" conf_num="' + conf_num + '" conf_size="' + conf_size + '"><span><b>' + 'Nº ' + conf_num + ' - ' + conf_name + ': </b></span><span class="value config" value="[]"></span><div class="configuration_updated"><span class="intl" id="updated"></span>: <span id="update_time"></span></div><div class="configuration_default"></div><div class="configuration_description"><i>' + conf_description + '</i></div></div>');
	...
	if (0x7a in node.instances[0].commandClasses) {
		fwupdate_cont = $('<div class="fwupdate_cont"><h4 class="cut_link">' + $.translate('fwupdate_title') + '</h4><div class="content"><button id="fw_update" class="intl" device="' + nodeId + '"></button><br/><input type="text" id="fw_url" placeholder="URL"/><input type="text" id="fw_vendor_id" placeholder="Vendor id"/><input type="text" id="fw_id" placeholder="F/W id"/><input type="text" id="fw_target" placeholder="Target"/></div></div>').addClass('value wakeup');
		fwupdate_cont.find('#fw_update').bind('click', function() {
			runJS(
				'(function(fwurl, devId, VendorId, FWId, target) {' +
				'  console.log("Downloading FW...");' +
				'  http.request({' +
				'     url: fwurl,' +
				...
				'       zway.devices[devId].FirmwareUpdate.Perform(VendorId, FWId, target, res.data);' +
				'     },' +
				'     error: function(res) {' +
				'       console.logJS("Downloading FW... FAILED:", res.statusText);' +
				'     }' +
				'  });' +
				'})("' + $('#fw_url').val() + '", ' + $(this).attr('device') + ', ' + $('#fw_vendor_id').val() + ', ' + $('#fw_id').val() + ', ' + $('#fw_target').val() + ');'
			);
		});
	};
	...
	var advanced_xml_cont = $(
			'<div class="geek"><h4 class="cut_link">' + $.translate('advanced_xml_title') + '</h4>' +
				'<div class="content" style="display: none;">' + 
					...
					'<button id="config_ui_save_xml" class="intl geek"></button>' +
					'<br/>' +
	...
	div_cont.empty();
	div_cont.append(selectxml_cont).append(description_cont).append(assoc_cont).append(config_cont).append(switchall_cont).append(protection_cont).append(wakeup_cont).append(fwupdate_cont).append(import_export_save_config_cont).append(apply_cont).append(advanced_xml_cont);
	...
	div_cont.find('#config_ui_save_xml').bind('click', function() { window.open('/ZAutomation/CreateZDDX/' + nodeId, 'ZDDX for node ' + nodeId); return false; });
